...
import GradesService, { type CourseGrade, type CumulativeGradeReport, type GradeComponent } from "@/services/grades.service";
...
function ComponentRow({ comp, index }: { comp: GradeComponent; index: number }) {
  ...
  // Map component type to display
  const typeConfig = {
    ASSIGNMENT: { label: "Assignment", color: "bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400" },
    CLASS_GROUP: { label: "Class Group", color: "bg-violet-50 dark:bg-violet-950/30 text-violet-600 dark:text-violet-400" },
    QUIZ: { label: "Quiz", color: "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400" },
  };

  const typeInfo = typeConfig[comp.type as keyof typeof typeConfig] ?? { 
    label: comp.type, 
    color: "bg-gray-50 dark:bg-gray-950/30 text-gray-600 dark:text-gray-400" 
  };
  ...
  return (
    ...
    >
      {/* Type badge */}
      <span className={`flex-shrink-0 mt-0.5 text-[10px] font-bold uppercase px-2 py-0.5 rounded-lg ${typeInfo.color}`}>
        {typeInfo.label}
      </span>
...
export default function StudentGrades() {
  ...
  const gpa = data ? letterToGPA(data.overallLetterGrade) : 0;
  const overallPct = data ? Math.round(data.overallGrade ?? 0) : 0;
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            {isLoading 
              ...
              : `Cumulative grade report across ${data?.totalCourses ?? 0} enrolled course${data?.totalCourses !== 1 ? "s" : ""}`
            }
          </p>
        </div>
      </Fade>

      {/* Show skeleton only for data-dependent content */}
      {isLoading ? (
        <Fade delay={0.05}>
          <DataSkeleton />
        </Fade>
              ...
              <Card className="p-6 flex flex-col justify-center gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    sub: allComponents.length > 0 ? "graded items" : "no grades yet",
                  },
