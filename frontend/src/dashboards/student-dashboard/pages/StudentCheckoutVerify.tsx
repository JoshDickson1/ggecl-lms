    ...
    >
      <div className="rounded-[28px] p-10 text-center bg-white dark:bg-[#0f1623]
        ...
        <p className="text-gray-400 text-sm">
          Confirming your transaction with the payment provider…
        </p>
      </div>
    </motion.div>
  );
        ...
        <div className="rounded-[28px] p-10 text-center bg-white dark:bg-[#0f1623]
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            <p className="text-xs text-gray-400 mb-1">
              Order ID: <span className="font-bold text-blue-600 dark:text-blue-400">{orderId}</span>
            </p>
    ...
    >
      <div className="rounded-[28px] p-10 text-center bg-white dark:bg-[#0f1623]
        ...
        <p className="text-gray-400 text-sm mb-8">
          This link doesn't contain a valid order ID. If you just made a payment, check your email for confirmation.
        </p>
        <Link to="/student/cart">
          <motion.div
            whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function StudentCheckoutVerify() {
  const [params] = useSearchParams();
  const orderId = params.get("orderId");

  const { data: order, isLoading, error } = useOrderStatus(orderId, !!orderId);

  // Determine background accent colour based on resolved status
  const accentColor =
    !orderId || error
    ...
    <div className="min-h-screen bg-gray-50/60 dark:bg-[#080d18] flex items-center justify-center px-6 py-24">
      ...
      <div className="relative z-10 w-full flex justify-center">
        {!orderId ? (
          ...
          // Treat a fetch error as a failed order so the user can retry
