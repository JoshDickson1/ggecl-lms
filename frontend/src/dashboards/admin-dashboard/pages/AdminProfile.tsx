...
import {
  ...
  Shield, ShieldCheck, Activity, TrendingUp,
  Ticket, DollarSign, CheckCircle2, Clock,
  BarChart3, Zap,
} from "lucide-react";
...
const ADMIN = {
  ...
  // Platform health
  platformHealth: [
    { label: "Platform Uptime",     value: "99.97%", status: "good"    },
    { label: "Active Users (24h)",  value: "1,240",  status: "good"    },
    { label: "Open Tickets",        value: "7",      status: "warning" },
    { label: "Pending Approvals",   value: "3",      status: "warning" },
  ],
};
    ...
    <div className="max-w-[1100px] mx-auto space-y-6 pb-10">
          ...
          <div className="px-6 pb-6">
            <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-12 mb-5">
              ...
              <div className="flex items-center gap-2 px-4 py-2.5 rounded-2xl
                ...
                <span className="text-lg font-black text-gray-900 dark:text-white">{ad.uptime}</span>
      ...
      <Fade delay={0.06}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <StatTile icon={BarChart3} value={ad.uptime}              label="Platform Uptime"   sub="last 90 days"         />
        </div>
      </Fade>
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
          <div className="flex flex-col gap-5">
            ...
            <Fade delay={0.12}>
              <Card className="p-7">
                <SectionHead icon={Activity} title="Platform Health" />
                <div className="grid grid-cols-2 gap-3">
                  {ad.platformHealth.map((item, i) => (
                    <motion.div key={item.label}
                      ...
                      className={`flex items-center justify-between p-3.5 rounded-2xl border transition-colors ${
                        item.status === "good"
                          ...
                          : "bg-amber-50/60 dark:bg-amber-950/15 border-amber-100/60 dark:border-amber-900/20"
                      }`}>
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{item.label}</p>
                        <p className="text-lg font-black text-gray-900 dark:text-white mt-0.5">{item.value}</p>
                      </div>
                      <div className={`w-2 h-2 rounded-full ${item.status === "good" ? "bg-emerald-400" : "bg-amber-400"} animate-pulse`} />
                    </motion.div>
                  ))}
                </div>
              </Card>
            ...
            <Fade delay={0.16}>
              <Card className="p-7">
                ...
                <div className="flex flex-col gap-3">
                  {ad.recentActivity.slice(0, 4).map((a, i) => (
          ...
          <div className="flex flex-col gap-5">
            ...
            <Fade delay={0.18}>
              <Card className="p-5">
                <p className="text-[11px] font-bold tracking-widest text-gray-400 dark:text-gray-500 uppercase mb-3">Quick Access</p>
                <div className="flex flex-col gap-2">
                  {[
                    { label: "User Management",    to: "/dashboard/users",       icon: Users       },
                    { label: "Support Tickets",    to: "/dashboard/support",     icon: Ticket      },
                    { label: "Analytics",          to: "/dashboard/analytics",   icon: BarChart3   },
                    { label: "Platform Settings",  to: "/dashboard/settings",    icon: Shield      },
                  ].map(link => (
                    <Link key={link.to} to={link.to}
                      className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl
                        ...
                        transition-all group">
                      <div className="w-6 h-6 rounded-lg bg-gray-100 dark:bg-white/[0.06] flex items-center justify-center flex-shrink-0">
            ...
            <Fade key={i} delay={i * 0.05}>
              <Card className="p-4">
                <div className="flex items-center gap-4">
                  <ActivityIcon type={a.type} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 dark:text-white">{a.action}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{a.target}</p>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {[
            ...
            <Fade key={group.group} delay={gi * 0.07}>
              <Card className="p-5">
                <p className="text-[11px] font-bold tracking-widest text-gray-400 dark:text-gray-500 uppercase mb-3">{group.group}</p>
                <div className="flex flex-col gap-2">
