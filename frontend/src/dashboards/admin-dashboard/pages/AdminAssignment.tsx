...
import {
  ...
  Calendar, Award, Info, AlignLeft, Paperclip,
} from "lucide-react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import AssignmentService, { type UpdateAssignmentDto, type AssignmentResponse } from "@/services/assignment.service";
...
function getAttachmentType(url: string): "image" | "video" | "pdf" | "file" {
...
/** Ensures a URL is absolute so it doesn't get treated as a relative path by the browser. */

function AttachmentCard({ url, index }: { url: string; index: number }) {
  const type = getAttachmentType(url);
  const name = getFileName(url);
  const [imgError, setImgError] = useState(false);

  // Ensure the URL is absolute so it doesn't get treated as a relative path
  const safeUrl = url.startsWith("http://") || url.startsWith("https://") || url.startsWith("//")
    ? url
    : `https://${url}`;

  return (
    <div className="rounded-2xl border border-gray-100 dark:border-white/[0.08] overflow-hidden bg-gray-50 dark:bg-white/[0.03]">
      {/* Preview area */}
      {type === "image" && !imgError ? (
        <div className="relative group">
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
            <a href={safeUrl} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/90 text-gray-800 text-xs font-bold shadow hover:bg-white transition-all">
              <ExternalLink className="w-3.5 h-3.5" /> Open
            </a>
            <a href={safeUrl} download={name}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/90 text-gray-800 text-xs font-bold shadow hover:bg-white transition-all">
        ...
        <video src={safeUrl} controls className="w-full max-h-56 bg-black" />
      ) : type === "pdf" ? (
        ...
        <div className="flex items-center gap-3 p-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-gray-800 dark:text-white truncate">{name}</p>
            <p className="text-xs text-gray-400 mt-0.5">Attachment {index + 1}</p>
          </div>
        </div>
      )}

      {/* Action bar */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-t border-gray-100 dark:border-white/[0.06] bg-white dark:bg-[#0f1623]">
        <p className="flex-1 text-xs text-gray-500 dark:text-gray-400 truncate">{name}</p>
        <a href={safeUrl} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-all"
          title="Open in new tab">
          <ExternalLink className="w-3 h-3" /> View
        </a>
        <a href={safeUrl} download={name}
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/[0.06] transition-all"
          title="Download">
...
function AssignmentPreviewModal({
  assignment, courseName, onClose,
}: {
  assignment: AssignmentResponse & { submissions?: any[] };
  courseName: string;
  onClose: () => void;
}) {
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-[28px] bg-white dark:bg-[#0f1623] border border-gray-100 dark:border-white/[0.07] shadow-[0_32px_80px_rgba(0,0,0,0.28)] overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-start justify-between gap-4 px-7 py-5 border-b border-gray-100 dark:border-white/[0.07] flex-shrink-0">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-0.5 rounded-lg text-[11px] font-bold bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400">
                {courseName}
              </span>
              {assignment.allowLate && (
                <span className="px-2.5 py-0.5 rounded-lg text-[11px] font-bold bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400">
                  Late allowed
                </span>
              )}
            </div>
            <h2 className="text-xl font-black text-gray-900 dark:text-white leading-tight">{assignment.title}</h2>
          </div>
          <button onClick={onClose}
            className="w-8 h-8 rounded-xl flex items-center justify-center text-gray-400 hover:text-gray-700 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/[0.08] transition-all flex-shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              },
              {
                icon: Calendar,
                label: "Due Date",
                value: dueDate ? dueDate.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "No due date",
                color: isPast ? "text-red-500" : "text-emerald-600 dark:text-emerald-400",
                bg: isPast ? "bg-red-50 dark:bg-red-950/30" : "bg-emerald-50 dark:bg-emerald-950/30",
              },
              {
                icon: Users,
                label: "Submissions",
                value: `${(assignment as any).submissions?.length ?? 0}`,
          ...
          {assignment.description && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-xs font-bold tracking-widest text-gray-400 uppercase">Description</h3>
              </div>
          ...
          {assignment.instructions && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-xs font-bold tracking-widest text-gray-400 uppercase">Instructions</h3>
              </div>
          ...
          <div className="pt-2 border-t border-gray-100 dark:border-white/[0.06]">
            <p className="text-xs text-gray-400">
              Created {new Date(assignment.createdAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
              {" · "}ID: <span className="font-mono">{assignment.id}</span>
            </p>
          </div>
        </div>

        {/* Footer */}
        <div className="flex items-center justify-between gap-3 px-7 py-4 border-t border-gray-100 dark:border-white/[0.07] flex-shrink-0">
          <Link
            to={`/admin/assignments/${assignment.id}/submissions`}
            onClick={onClose}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold border border-indigo-200 dark:border-indigo-800/50 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 transition-all"
          >
            <Users className="w-4 h-4" /> View Submissions
          </Link>
          <button onClick={onClose}
            className="px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/[0.06] transition-all">
            Close
          </button>
        </div>
      </motion.div>
...
export default function AdminAssignment() {
  ...
  const [previewTarget, setPreviewTarget] = useState<(AssignmentResponse & { submissions?: any[] }) | null>(null);
      ...
      <AnimatePresence>
        ...
        {previewTarget && (
          <AssignmentPreviewModal
            ...
            courseName={courses.find((c: any) => c.id === previewTarget.courseId)?.title ?? "Unknown Course"}
